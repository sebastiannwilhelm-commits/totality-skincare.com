...
let signupFormGoogleAutoOnce = false;
...
export function SignupForm() {
  ...
  React.useEffect(() => {
    ...
    async function checkRedirectResult() {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : "Unable to finish Google sign-up.";
          setError(message);
        }
      }
    }
  ...
  React.useEffect(() => {
    ...
    if (signupFormGoogleAutoOnce) return;
    signupFormGoogleAutoOnce = true;
    void onGoogleSignUp();
  }, [googleAuto, resolvedConfigured, onGoogleSignUp]);
  ...
  return (
    <form onSubmit={onSubmit} className="mx-auto max-w-sm space-y-4">
      ...
      <Button
        ...
        className="w-full"
        variant="outline"
        disabled={fieldDisabled || googleLoading}
